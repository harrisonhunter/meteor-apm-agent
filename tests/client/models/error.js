Tinytest.add('Client Side - Error Model - sends errors', function (test) {
...
Tinytest.add('Client Side - Error Model - sends same error twice', function (
  test
) {
  var em = new ErrorModel();
  var payloadReceived;
...
Tinytest.add('Client Side - Error Model - isErrorExists', function (test) {
  var em = new ErrorModel();
  var payloadReceived;
  var resetSend = onKadiraSend(function (payload) {
    resetSend();
...
Tinytest.add('Client Side - Error Model - increamentErrorCount', function (
  test
) {
  var em = new ErrorModel();
  var payloadReceived;
...
Tinytest.add('Client Side - Error Model - canSendErrors', function (test) {
...
Tinytest.addAsync('Client Side - Error Model - validateInterval', function (
  test,
  done
) {
  var em = new ErrorModel({
...
Tinytest.add(
  ...
  function (test) {
    ...
    test.equal(payloadReceived, {
      errors: [{ name: 'hello', count: 1, startTime: 600 }],
      host: undefined,
    });
