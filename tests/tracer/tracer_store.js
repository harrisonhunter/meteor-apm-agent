Tinytest.add('TracerStore - ._getMean', function (test) {
  var ts = new TracerStore();
  var mean = ts._getMean([10, 20, 30]);
  test.equal(mean, 20);
});

Tinytest.add('TracerStore - ._getMedian', function (test) {
  var ts = new TracerStore();
  var median = ts._getMedian([10, 20, 30, 40, 1, 3]);
  test.equal(median, 15);
});

Tinytest.add('TracerStore - ._calculateMad', function (test) {
  var ts = new TracerStore();
...
Tinytest.add('TracerStore - ._isOutlier', function (test) {
  var ts = new TracerStore();
  var dataSet = [10, 20, 30, 40, 1, 3, 3];
...
Tinytest.add('TracerStore - lower outlier', function (test) {
  var ts = new TracerStore();
...
Tinytest.add('TracerStore - addTrace - fresh add', function (test) {
  var ts = new TracerStore();
  ts.addTrace({ name: 'one', type: 'method', metrics: { total: 100 } });
  var currentMaxTrace = ts.currentMaxTrace;
  test.equal(currentMaxTrace, {
    'method::one': { name: 'one', type: 'method', metrics: { total: 100 } },
  });
});

Tinytest.add('TracerStore - addTrace - second time higher total', function (
  test
) {
  var ts = new TracerStore();
  ts.addTrace({ name: 'one', type: 'method', metrics: { total: 100 } });
  ...
  var currentMaxTrace = ts.currentMaxTrace;
  test.equal(currentMaxTrace, {
    'method::one': { name: 'one', type: 'method', metrics: { total: 200 } },
  });
});

Tinytest.add('TracerStore - addTrace - second time lower total', function (
  test
) {
  var ts = new TracerStore();
  ts.addTrace({ name: 'one', type: 'method', metrics: { total: 100 } });
...
Tinytest.add('TracerStore - processTraces - process at first', function (test) {
...
Tinytest.add('TracerStore - processTraces - no traces', function (test) {
...
Tinytest.add('TracerStore - processTraces - maxTotalPoints', function (test) {
  var ts = new TracerStore({ maxTotalPoints: 3 });
  ts.addTrace({ name: 'one', type: 'method', metrics: { total: 100 } });
  ts.processTraces();
  ts.addTrace({ name: 'one', type: 'method', metrics: { total: 200 } });
...
Tinytest.add('TracerStore - _handleErrors - single error', function (test) {
  var ts = new TracerStore({ archiveEvery: 20 });
  var trace = {
    name: 'one',
    type: 'method',
    events: [['start'], ['end', 0, { error: { message: 'ERROR_MESSAGE' } }]],
  };
  var ts = new TracerStore();
...
Tinytest.add('TracerStore - _handleErrors - single error no data', function (
  test
) {
  var ts = new TracerStore({ archiveEvery: 20 });
  var trace = { name: 'one', type: 'method', events: [['start'], ['end', 0]] };
  var ts = new TracerStore();
  ts._handleErrors(trace);
  test.equal(ts.traceArchive, []);
});

Tinytest.add('TracerStore - _handleErrors - multiple error', function (test) {
  var trace = {
    name: 'one',
    type: 'method',
    events: [['start'], ['end', 0, { error: { message: 'ERROR_MESSAGE' } }]],
