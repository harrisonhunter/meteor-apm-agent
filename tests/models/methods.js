Tinytest.add('Models - Method - buildPayload simple', function (test) {
  ...
  test.equal(
    EJSON.parse(EJSON.stringify(payload)),
    EJSON.parse(EJSON.stringify(expected))
  );
  CleanTestData();
});

Tinytest.add('Models - Method - buildPayload with errors', function (test) {
  CreateMethodCompleted('aa', 'hello', 1, 100, 5);
  ...
  test.equal(
    EJSON.parse(EJSON.stringify(payload.methodMetrics)),
    EJSON.parse(EJSON.stringify(expected))
  );
  CleanTestData();
});

Tinytest.add('Models - Method - Metrics - fetchedDocSize', function (test) {
...
Tinytest.add('Models - Method - Metrics - sentMsgSize', function (test) {
  var docs = [{ data: 'data1' }, { data: 'data2' }];
  docs.forEach(function (doc) {
    TestData.insert(doc);
  });
  ...
  var expected = (
    JSON.stringify({ msg: 'updated', methods: ['1'] }) +
    JSON.stringify({ msg: 'result', id: '1', result: returnValue })
  ).length;
...
function CreateMethodCompleted(
  sessionName,
  methodName,
  methodId,
  startTime,
...
function CreateMethodWithEvent(
  sessionName,
  methodName,
  methodId,
  startTime,
