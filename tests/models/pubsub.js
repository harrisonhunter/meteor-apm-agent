Tinytest.add('Models - PubSub - Metrics - same date', function (test) {
  var pub = 'postsList';
...
Tinytest.add('Models - PubSub - Metrics - multi date', function (test) {
  var pub = 'postsList';
  ...
  var metrics = [model._getMetrics(d1, pub), model._getMetrics(d2, pub)];
  test.equal(metrics[0].subs, 2);
  test.equal(metrics[1].subs, 1);
});

Tinytest.add('Models - PubSub - Metrics - same minute', function (test) {
  var pub = 'postsList';
  var d1 = new Date('2013 Dec 10 20:31:12').getTime();
...
Tinytest.add('Models - PubSub - BuildPayload - subs', function (test) {
  var pub = 'postsList';
  var d1 = new Date('2013 Dec 10 20:31:12').getTime();
  var model = new PubsubModel();
  model._getMetrics(d1, pub).subs++;
  model._getMetrics(d1, pub).subs++;
  model._getMetrics(d1, pub).subs++;
  var metrics = [model.buildPayload(), model.metricsByMinute];
  test.equal(metrics[0].pubMetrics[0].pubs.postsList.subs, 3);
  test.equal(metrics[1], {});
});

Tinytest.add('Models - PubSub - BuildPayload - routes', function (test) {
  var pub = 'postsList';
  var d1 = new Date('2013 Dec 10 20:31:12').getTime();
  var route = 'route1';
  var model = new PubsubModel();
...
Tinytest.add('Models - PubSub - BuildPayload - response time', function (test) {
  var pub = 'postsList';
  var d1 = new Date('2013 Dec 10 20:31:12').getTime();
  var model = new PubsubModel();
...
Tinytest.add('Models - PubSub - BuildPayload - lifetime', function (test) {
  var pub = 'postsList';
  var d1 = new Date('2013 Dec 10 20:31:12').getTime();
  var model = new PubsubModel();
  var metrics = model._getMetrics(d1, pub);
...
Tinytest.add('Models - PubSub - BuildPayload - multiple dates', function (
  test
) {
  var d1 = new Date('2013 Dec 10 20:31:12').getTime();
...
Tinytest.add('Models - PubSub - Observer Cache - no cache', function (test) {
...
Tinytest.add('Models - PubSub - Observer Cache - single cache', function (
  test
) {
  var original = Kadira.syncedDate.getTime;
  var dates = [
...
Tinytest.add('Models - PubSub - Observer Cache - multiple dates', function (
  test
) {
...
Tinytest.add('Models - PubSub - ActiveDocs - Single Sub - simple', function (
  test
) {
  CleanTestData();
...
Tinytest.add('Models - PubSub - ActiveDocs - Multiple Subs - simple', function (
  test
) {
  CleanTestData();
  var docs = [{ data: 'data1' }, { data: 'data2' }, { data: 'data3' }];
...
Tinytest.add('Models - PubSub - Observers - simple', function (test) {
  CleanTestData();
  var client = GetMeteorClient();
  var h1 = SubscribeAndWait(client, 'tinytest-data');
  Wait(200);
...
Tinytest.add(
  ...
  function (test) {
    ...
    var payload = GetPubSubPayload();
    test.equal(
      payload[0].pubs['tinytest-data-with-no-oplog'].polledDocuments,
      2
    );
    h1.stop();
    CloseClient(client);
...
Tinytest.add('Models - PubSub - Observers - initiallyAddedDocuments', function (
  test
) {
  CleanTestData();
  var client = GetMeteorClient();
  ...
  var payload = GetPubSubPayload();
  test.equal(
    payload[0].pubs['tinytest-data-random'].initiallyAddedDocuments,
    4
  );
  CloseClient(client);
});

Tinytest.add('Models - PubSub - Observers - liveAddedDocuments', function (
  test
) {
  CleanTestData();
  var client = GetMeteorClient();
...
Tinytest.add('Models - PubSub - Observers - liveChangedDocuments', function (
  test
) {
  CleanTestData();
  var client = GetMeteorClient();
...
Tinytest.add('Models - PubSub - Observers - liveRemovedDocuments', function (
  test
) {
  CleanTestData();
  var client = GetMeteorClient();
...
Tinytest.add('Models - PubSub - Observers - initiallySentMsgSize', function (
  test
) {
  CleanTestData();
  var client = GetMeteorClient();
  ...
  var templateMsg =
    '{"msg":"added","collection":"tinytest-data","id":"17digitslongidxxx","fields":{"aa":10}}';
  var expectedMsgSize = templateMsg.length * 2;

  test.equal(
    payload[0].pubs['tinytest-data-random'].initiallySentMsgSize,
    expectedMsgSize
  );
  CloseClient(client);
});

Tinytest.add('Models - PubSub - Observers - liveSentMsgSize', function (test) {
  CleanTestData();
  var client = GetMeteorClient();
  var h1 = SubscribeAndWait(client, 'tinytest-data-random');
  Wait(50);
...
Tinytest.add('Models - PubSub - Observers - initiallyFetchedDocSize', function (
  test
) {
  CleanTestData();
  var client = GetMeteorClient();
  ...
  var payload = GetPubSubPayload();
  test.equal(
    payload[0].pubs['tinytest-data-random'].initiallyFetchedDocSize,
    60
  );
  CloseClient(client);
});

Tinytest.add('Models - PubSub - Observers - liveFetchedDocSize', function (
  test
) {
  CleanTestData();
  var client = GetMeteorClient();
...
Tinytest.add('Models - PubSub - Observers - fetchedDocSize', function (test) {
  CleanTestData();
  var client = GetMeteorClient();
  TestData.insert({ aa: 10 });
  TestData.insert({ aa: 20 });
...
Tinytest.add('Models - PubSub - Observers - polledDocSize', function (test) {
  CleanTestData();
  var client = GetMeteorClient();
  TestData.insert({ aa: 10 });
  TestData.insert({ aa: 20 });
