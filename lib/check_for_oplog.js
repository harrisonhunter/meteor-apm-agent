...
OplogCheck.env = function () {
  if (!process.env.MONGO_OPLOG_URL) {
    return {
      ...
      solution:
        'Add oplog support for your Meteor app. see: http://goo.gl/Co1jJc',
    };
  } else {
    return true;
  }
...
OplogCheck.disableOplog = function (cursorDescription) {
  if (cursorDescription.options._disableOplog) {
    return {
      code: 'DISABLE_OPLOG',
      reason:
        "You've disable oplog for this cursor explicitly with _disableOplog option.",
    };
  } else {
    return true;
  }
...
OplogCheck.skip = function (cursorDescription) {
  if (cursorDescription.options.skip) {
    return {
      ...
      solution:
        'Try to avoid using skip. Use range queries instead: http://goo.gl/b522Av',
    };
  }
...
OplogCheck.geo = function (cursorDescription) {
  ...
  if (matcher.hasGeoQuery()) {
    return {
      ...
      solution:
        'Try to remove geo partial operators from your query if possible.',
    };
  }
...
OplogCheck.limitButNoSort = function (cursorDescription) {
  ...
  if (options.limit && !options.sort) {
    return {
      code: 'LIMIT_NO_SORT',
      reason:
        'Meteor oplog implementation does not support limit without a sort specifier.',
...
OplogCheck.gitCheckout = function (cursorDescription, driver) {
  if (!Meteor.release) {
    return {
      code: 'GIT_CHECKOUT',
      reason:
        "Seems like your Meteor version is based on a Git checkout and it doesn't have the oplog support.",
...
Kadira.checkWhyNoOplog = function (cursorDescription, observerDriver) {
  if (typeof Minimongo == 'undefined') {
    return {
      code: 'CANNOT_DETECT',
      reason:
        "You are running an older Meteor version and Kadira can't check oplog state.",
      solution: 'Try updating your Meteor app',
    };
  }

  var result = runMatchers(
    preRunningMatchers,
    cursorDescription,
    observerDriver
  );
