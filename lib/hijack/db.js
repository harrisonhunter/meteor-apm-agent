...
hijackDBOps = function hijackDBOps() {
  ...
  ['find', 'update', 'remove', 'insert', '_ensureIndex', '_dropIndex'].forEach(
    function (func) {
  ...
  [
    ...
    cursorProto[type] = function () {
      ...
      if (cursorDescription.options) {
        var cursorOptions = _.pick(cursorDescription.options, [
          'fields',
          'sort',
          'limit',
        ]);
        ...
        if (type == 'observeChanges' || type == 'observe') {
          ...
          if (ret._multiplexer) {
            ...
            if (observerDriver) {
              ...
              var usesOplog =
                typeof observerDriverClass.cursorSupported == 'function';
          ...
          if (!endData.oplog) {
            ...
            var reasonInfo = Kadira.checkWhyNoOplog(
              cursorDescription,
              observerDriver
            );
          ...
          if (type == 'fetch') {
            ...
            var docSize =
              Kadira.docSzCache.getSize(coll, query, opts, ret) * ret.length;
            endData.docSize = docSize;

            if (kadiraInfo) {
              if (kadiraInfo.trace.type === 'method') {
                Kadira.models.methods.trackDocSize(
                  kadiraInfo.trace.name,
                  docSize
                );
