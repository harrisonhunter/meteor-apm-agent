wrapOplogObserveDriver = function (proto) {
  ...
  proto._publishNewResults = function (newResults, newBuffer) {
    ...
    if (this._ownerInfo) {
      ...
      Kadira.models.pubsub.trackDocSize(
        this._ownerInfo.name,
        'polledFetches',
        docSize * count
      );
    } else {
  ...
  var originalHandleOplogEntrySteadyOrFetching =
    proto._handleOplogEntrySteadyOrFetching;
  ...
  // track live updates
  ['_addPublished', '_removePublished', '_changePublished'].forEach(function (
    fnName
  ) {
...
wrapPollingObserveDriver = function (proto) {
  ...
  proto._pollMongo = function () {
    ...
    if (this._results && this._results.size) {
      ...
      docSize =
        Kadira.docSzCache.getSize(coll, query, opts, this._results._map) *
        count;
    }
...
wrapForCountingObservers = function () {
  ...
  mongoConnectionProto._observeChanges = function (
    cursorDescription,
    ordered,
    callbacks
  ) {
    var ret = originalObserveChanges.call(
      this,
      cursorDescription,
      ordered,
      callbacks
      ...
      if (!ret._multiplexer.__kadiraTracked) {
        ...
        if (kadiraInfo.trace.type == 'sub') {
          ...
          if (observerDriver._polledDocuments) {
            Kadira.models.pubsub.trackPolledDocuments(
              ownerInfo,
              observerDriver._polledDocuments
            );
